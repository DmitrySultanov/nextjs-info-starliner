...
export async function getPostsByCategoryName(categoryName, first = 9, before) {
  const data = await fetchAPI(
    `
    query postsByCategoryId($categoryName: String, $first: Int!, $before: String) {
      posts(first: $first, before: $before, where: {categoryName: $categoryName, orderby: { field: DATE, order: DESC }}) {
        edges {
          node {
            id
            date
